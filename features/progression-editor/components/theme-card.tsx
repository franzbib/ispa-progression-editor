...
export function ThemeCard({
  ...
  return (
    <article
      ...
      style={style}
      className={`rounded-md border bg-white p-4 shadow-panel print:break-inside-avoid ${
        isSelected ? "border-[#2c6b57]" : "border-[#d9ddd2]"
    ...
    >
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="flex min-w-0 flex-1 gap-3">
          <button
            className="mt-1 h-8 w-8 shrink-0 rounded-md border border-[#cbd3c4] bg-[#f7f8f5] text-sm font-semibold text-[#596257] print:hidden"
            type="button"
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#7a5147]">
              Thème {index + 1}
              {visibleIndex !== index ? ` · résultat ${visibleIndex + 1}` : ""}
            </p>
            <EditableText
              ariaLabel={`Renommer le thème ${theme.themeLabel}`}
              className="mt-1 w-full rounded-md border border-transparent bg-transparent px-1 py-1 text-xl font-semibold outline-none hover:border-[#d9ddd2] focus:border-[#2c6b57]"
      ...
      <label className="mt-3 block text-sm font-medium text-[#596257]">
        Notes du thème
        <textarea
          className="mt-1 min-h-16 w-full rounded-md border border-[#d9ddd2] bg-[#fbfcf8] px-3 py-2 text-sm outline-none focus:border-[#2c6b57]"
          value={notesDraft}
          ...
          >
            <div className="space-y-2">
              {theme.grammarPoints.length === 0 ? (
                <div className="rounded-md border border-dashed border-[#cbd3c4] bg-[#fbfcf8] px-3 py-5 text-sm text-[#697267]">
...
function GrammarRow({
  ...
  return (
    <div
      ref={sortable.setNodeRef}
      style={style}
      className={`grid gap-2 rounded-md border border-[#d9ddd2] bg-[#fbfcf8] px-3 py-2 md:grid-cols-[auto_1fr_auto] md:items-center ${
        sortable.isDragging ? "opacity-70" : ""
      }`}
    >
      <button
        className="h-7 w-7 rounded-md border border-[#cbd3c4] bg-white text-xs font-semibold text-[#596257] print:hidden"
        type="button"
      ...
      <div className="min-w-0">
        <span className="text-xs font-semibold text-[#7a5147]">{pointIndex + 1}.</span>
        <EditableText
          ariaLabel={`Renommer le point ${point.label}`}
          className="ml-1 w-[calc(100%-2rem)] rounded-md border border-transparent bg-transparent px-1 py-1 text-sm outline-none hover:border-[#d9ddd2] focus:border-[#2c6b57]"
          value={point.label}
          onCommit={onRename}
        />
      </div>
      <div className="flex flex-wrap gap-2 print:hidden">
        ...
        <select
          className="rounded-md border border-[#cbd3c4] bg-white px-2 py-1 text-xs"
